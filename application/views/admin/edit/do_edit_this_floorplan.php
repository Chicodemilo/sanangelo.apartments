<div class="hours_box">
		...
		<form action="<?php echo base_url() ?>admin/submit_floorplan_edits/<?php echo $apt_id; ?>/<?php echo $floorplan_info[0]['id']; ?>" method="post">
		<?php 
			$csrf = array(
		...
		<input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" />
			...
			<table class="floorplan_table_lower">
				...
				<tr>
					...
					<td><a href="<?php echo base_url() ?>admin/upload_this/<?php echo $apt_id; ?>/<?php echo $floorplan_info[0]['id'] ?>">Upload Diagram</a></td>
				</tr>
			</table>
			</div>
			...
			if($floorplan_info[0]['floorplan_pic'] != ''){
				echo	'<div class="floorplans_pic">
						<img src="'.base_url().'images/floorplans/'.$apt_id."/".$floorplan_info[0]['id'].'/'.$floorplan_info[0]['floorplan_pic'].'" alt="'.$floorplan_info[0]['name'].'" width="500px">
						<br>
						<a href="'.base_url().'admin/upload_this/'.$apt_id.'/'.$floorplan_info[0]['id'].'" class="not_fancy" style="font-size: .6em;">Upload Another Diagram</a>
						<br>
						<a href="'.base_url().'admin/delete_this_diagram/'.$apt_id.'/'.$floorplan_info[0]['id'].'" class="not_fancy" style="font-size: .6em;">Delete This Diagram</a>
					</div>';
			}
			?>
		</form>
