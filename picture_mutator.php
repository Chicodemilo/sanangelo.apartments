<html>
	<!-- <img src="images/pictures/2/38/test_pic_13.jpg" alt="">	 -->
...
$img_dir = scandir('images/pictures/');

foreach ($img_dir as $key => $value) {
	if($key != 0 && $key != 1 ){
		// echo "<br>--".$key.":".$value."<br>";
		if(is_dir('images/pictures/'.$value)){
			$img_dir_b = scandir('images/pictures/'.$value);

			foreach ($img_dir_b as $key_b => $value_b) {
				if($key_b != 0 ){
					// echo "<br>-----".$key_b.":".$value_b."<br>";

					if(is_dir('images/pictures/'.$value.'/'.$value_b)){
						$pics = scandir('images/pictures/'.$value.'/'.$value_b);
						echo count($pics)."<br>";
						if(count($pics) < 4){
							foreach ($pics as $key_c => $value_c) {

								$pic_parts = explode('.', $value_c);
								if($pic_parts[count($pic_parts) -1] == 'jpg' || $pic_parts[count($pic_parts) -1] == 'JPG'){
									echo "-----------".$value_c."<br>";
									$file = "images/pictures/".$value."/".$value_b."/".$value_c;
									echo $file;
									...
									$exp_type = explode(".", $exp_file[(count($exp_file) - 1)]);
									$type = $exp_type[count($exp_type) - 1];

									while($x < (count($exp_file) - 1 )){
										$front_path .= $exp_file[$x];
										if($x <= (count($exp_file) - 1)){
											$front_path .= "/";
										}
										$x++;
									}
